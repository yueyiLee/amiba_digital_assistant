/**
 * ai/tools.ts — AI 工具定义（Function Calling）与执行函数注册表
 *
 * 核心设计：工具不直接操作数据库，而是通过 apiClient 调用已有的 /api/* 路由。
 */
import * as api from './api-client';
import type { ToolHandler, ToolHandlerResult, ToolHandlersRegistry } from '../types/ai';
...
const TOOL_HANDLERS: ToolHandlersRegistry = {
  // ---- 经营看板 ----
  async get_dashboard(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const period: string = (params.period as string) || 'month';
    const now: Date = new Date();
    let startDate: string, endDate: string, monthStr: string | undefined;
    ...
    const query: Record<string, unknown> = { startDate, endDate };
    ...
    if (txResp.status >= 400) {
      return { success: false, message: (txResp.data as Record<string, unknown>).error as string || '获取收支数据失败' };
    }
    const transactions = txResp.data as Record<string, unknown>[];

    let workHours: Record<string, unknown>[] = [];
    if (monthStr) {
      const whResp = await api.get('/api/workhours', { month: monthStr }, token);
      if (whResp.status < 400) workHours = whResp.data as Record<string, unknown>[];
    }

    const sumByType = (type: string): number =>
      transactions.filter((t) => t.type === type).reduce((s, t) => s + Math.abs(t.amount as number), 0);
    ...
    const totalHours = workHours.reduce((s, w) => s + ((w.hours as number) || 0), 0);
    const totalSalary = workHours.reduce((s, w) => s + ((w.hours as number) || 0) * ((w.hourly_rate as number) || 0), 0);
  ...
  // ---- 收支流水 ----
  async get_transactions(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    ...
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    const rows = resp.data as Record<string, unknown>[];
    return { success: true, data: rows, message: `找到 ${rows.length} 条收支记录` };
  },

  async add_transaction(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    ...
    if (params.customer_name) {
      ...
      if (custResp.status < 400) {
        const found = (custResp.data as Record<string, unknown>[]).find((c) => c.name === params.customer_name);
    ...
    if (params.product_name) {
      ...
      if (prodResp.status < 400) {
        const found = (prodResp.data as Record<string, unknown>[]).find((p) => p.name === params.product_name);
    ...
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    return { success: true, data: resp.data as Record<string, unknown>, message: `已新增收支记录：${params.type} ${params.amount}元` };
  },

  async update_transaction(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const body: Record<string, unknown> = {};
  ...
  async delete_transaction(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
  ...
  // ---- 客户 ----
  async get_customers(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const resp = await api.get('/api/customers', {}, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    let rows = resp.data as Record<string, unknown>[];
    if (params.keyword) {
      const kw: string = (params.keyword as string).toLowerCase();
      rows = rows.filter((c) => (c.name as string).toLowerCase().includes(kw));
    }
    return { success: true, data: rows, message: `找到 ${rows.length} 个客户` };
  },

  async add_customer(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const body: Record<string, unknown> = { name: params.name, type: params.type, contact: params.contact || '', address: params.address || '' };
    const resp = await api.post('/api/customers', body, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    return { success: true, data: resp.data as Record<string, unknown>, message: `已新增客户：${params.name}` };
  },

  async update_customer(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const body: Record<string, unknown> = {};
  ...
  async delete_customer(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
  ...
  // ---- 商品 ----
  async get_products(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const resp = await api.get('/api/products', {}, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    let rows = resp.data as Record<string, unknown>[];
    if (params.keyword) {
      const kw: string = (params.keyword as string).toLowerCase();
      rows = rows.filter((p) => (p.name as string).toLowerCase().includes(kw) || ((p.brand as string) || '').toLowerCase().includes(kw));
    }
    return { success: true, data: rows, message: `找到 ${rows.length} 个商品` };
  },

  async add_product(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const body: Record<string, unknown> = {
      name: params.name, brand: params.brand || '', unit: params.unit || '件',
      category1: params.category1, category2: params.category2 || '',
      purchase_price: params.purchase_price || 0, sale_price: params.sale_price || 0,
    };
    const resp = await api.post('/api/products', body, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    return { success: true, data: resp.data as Record<string, unknown>, message: `已新增商品：${params.name}` };
  },

  async update_product(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const body: Record<string, unknown> = {};
    if (params.name !== undefined) body.name = params.name;
  ...
  async delete_product(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
  ...
  // ---- 库存 ----
  async get_inventory(_params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const resp = await api.get('/api/inventory', {}, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    return { success: true, data: resp.data as Record<string, unknown>, message: `共 ${(resp.data as unknown[]).length} 条库存记录` };
  },

  async update_inventory(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const body: Record<string, unknown> = { product_id: params.product_id, quantity: params.quantity };
  ...
  // ---- 合同 ----
  async get_contracts(_params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const resp = await api.get('/api/contracts', {}, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    return { success: true, data: resp.data as Record<string, unknown>, message: `共 ${(resp.data as unknown[]).length} 个合同` };
  },

  async add_contract(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const body: Record<string, unknown> = {
      contract_no: params.contract_no, customer_id: params.customer_id, amount: params.amount,
      status: params.status || '进行中', start_date: params.start_date || '', end_date: params.end_date || '',
      note: params.note || '',
    };
    const resp = await api.post('/api/contracts', body, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    return { success: true, data: resp.data as Record<string, unknown>, message: `已新增合同：${params.contract_no}` };
  },

  // ---- 员工 ----
  async get_employees(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const resp = await api.get('/api/employees', {}, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    let rows = resp.data as Record<string, unknown>[];
    if (params.status && params.status !== 'all') {
      rows = rows.filter((e) => ((e.status as string) || 'active') === params.status);
    }
    return { success: true, data: rows, message: `找到 ${rows.length} 名员工` };
  },

  async add_employee(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const body: Record<string, unknown> = {
      name: params.name, position: params.position || '', hourly_rate: params.hourly_rate,
      join_date: params.join_date || '',
    };
    const resp = await api.post('/api/employees', body, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    return { success: true, data: resp.data as Record<string, unknown>, message: `已新增员工：${params.name}（${params.position || '未设岗位'}，时薪${params.hourly_rate}元）` };
  },

  async set_employee_status(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const body: Record<string, unknown> = { status: params.status, leave_date: params.leave_date || '', note: params.note || '' };
    const resp = await api.patch(`/api/employees/${params.employee_id}/status`, body, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    const action: string = params.status === 'left' ? '离职' : '复职';
    return { success: true, message: `已办理员工 #${params.employee_id} ${action}` };
  },

  async record_work_hours(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const body: Record<string, unknown> = { employee_id: params.employee_id, hours: params.hours, month: params.month };
  ...
  async get_work_hours(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const query: Record<string, unknown> = {};
    if (params.month) query.month = params.month;
    const resp = await api.get('/api/workhours', query, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    return { success: true, data: resp.data as Record<string, unknown>, message: `找到 ${(resp.data as unknown[]).length} 条工时记录` };
  },

  // ---- 经营分析摘要 ----
  async get_analysis_summary(params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const query: Record<string, unknown> = {};
    if (params.startDate) query.startDate = params.startDate;
    if (params.endDate) query.endDate = params.endDate;
    if (params.unit) query.unit = params.unit;
    const resp = await api.get('/api/transactions', query, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    const txns = resp.data as Record<string, unknown>[];

    const sumByType = (type: string): number =>
      txns.filter((t) => t.type === type).reduce((s, t) => s + Math.abs(t.amount as number), 0);
  ...
  // ---- 设置 ----
  async get_settings(_params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const resp = await api.get('/api/settings', {}, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    return { success: true, data: resp.data as Record<string, unknown>, message: '当前账号设置' };
  },

  async get_expense_types(_params: Record<string, unknown>, token: string): Promise<ToolHandlerResult> {
    const resp = await api.get('/api/expense-types', {}, token);
    if (resp.status >= 400) return { success: false, message: (resp.data as Record<string, unknown>).error as string };
    return { success: true, data: resp.data as Record<string, unknown>, message: `共 ${(resp.data as unknown[]).length} 个收支类型` };
  },
};
