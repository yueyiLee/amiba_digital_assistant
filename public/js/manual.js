...
const Manual = (() => {
  ...
  const TOC = [
    { id: 'ch1', t: '新手上路：第一次使用' },
    { id: 'ch2', t: '管好客户与合同（前置基础）' },
    { id: 'ch3', t: '记好每一笔账（收支管理）' },
    { id: 'ch4', t: '商品、服务与库存' },
    { id: 'ch5', t: '看清经营状况' },
    { id: 'ch6', t: '算清阿米巴的账' },
    { id: 'ch7', t: '管人与发薪（员工管理）' },
    { id: 'ch8', t: 'AI 经营助手提效' },
    { id: 'ch9', t: '系统设置与账号（管理员）' }
  ];
      ...
      <div class="manual-wrap">
        ...
        <article class="manual-content">
          ...
          <section id="ch1" class="manual-ch">
            ...
            ${warn('账号由系统管理员（admin）创建并分配。若提示「用户名或密码错误」，请联系管理员确认账号或重置密码。')}
          ...
          <section id="ch5" class="manual-ch">
            ...
            ${shot('shot-analysis', '经营分析：多维度图表下钻')}
          ...
          <section id="ch9" class="manual-ch">
            <h2>⑨ 系统设置与账号（管理员）</h2>
            <p>以下多为<strong>管理员（admin）</strong>可见；普通账号无「账号管理」入口。</p>

            <h3>9.1 基础数据维护</h3>
            <ul>
              <li><strong>经营设置 ▸ 部门设置</strong>：增删部门，支撑独立核算。</li>
              <li><strong>经营设置 ▸ 收支类型</strong>：自定义收入/支出类型（如销售收款、房租）。</li>
              <li><strong>经营设置 ▸ 杂费类别设置</strong>：维护杂费类别与联动规则。</li>
              <li><strong>经营设置 ▸ 显示与导出设置</strong>：设置默认币种、导出选项等。</li>
            </ul>
            ${shot('shot-settings', '经营设置：类型、杂费与显示导出')}

            <h3>9.2 账号管理</h3>
            <ol>
              <li>左侧导航 <strong>账号管理</strong>（仅管理员）。</li>
              <li>查看账号列表，<strong>创建新账号</strong>、停用/启用账号。</li>
              <li>每个账号绑定唯一<strong>企业名称</strong>，显示在首页账号旁。</li>
            </ol>
            ${shot('shot-users', '账号管理：创建账号、绑定企业')}
            ${warn('账号按「owner_id」隔离数据：每个账号只能看到自己录入的数据。管理员账号可管理所有租户账号，但业务数据仍各自隔离。')}

            <h3>9.3 修改自己的密码</h3>
            <ol>
              <li>在登录态下使用「修改密码」功能，输入原密码与新密码（至少 6 位）即可。</li>
            </ol>
